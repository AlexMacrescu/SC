...
class AES
{/*s box matrix : The AES Substitution Table */// 256-bit key with 256-bit support length of 32 characters
	...
	static void MixColumns(unsigned char* col) // Column mix
	{
	...
	static void SubBytes(unsigned char* col) // byte substitution
	{
		int x;
		for (x = 0; x < 16; x++)
		{
	...
	static void AddRoundKey(unsigned char* col, unsigned char* expansionkey, int round) // Key add
	{
		int x;
		for (x = 0; x < 16; x++)
		{
	...
	void ScheduleKey(unsigned char* inkey, unsigned char* outkey, int Nk, int Nr) // arrange a secret key to use
	{
